...
#define MAX_SAMPLES 300 // Ensure this is low enough to not occupy all available memory

// Stack allocated arrays of size 4 * MAX_SAMPLES - (sizeof(float) == 4 bytes)
...
float adjust_sample_rate(float dominant_freq){
  ...
  if (f < 0.5) return 0.5;
  if (f > 4.0) return 4.0;
  Serial.print("Sample rate adjusted: "); Serial.print(f);
  return f;
}

void get_dominant_frequency(int sample_count, float* out){
  float max_mag = 0;
  float dom_freq = 0;
  ...
  // You cant 'return a pointer if not dynamically allocated, that would be returning a stack variable which would cause a crash,
  // using 'malloc/calloc on a single float is overkill and a waste of the heap
  *out = dom_freq;
}
...
void loop()
{
  ...
  float dom_freq;
  get_dominant_frequency(N, &dom_freq);
